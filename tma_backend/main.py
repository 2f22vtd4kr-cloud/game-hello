...
async def ai_chat(body: AiChatRequest, db: Session = Depends(get_db)):
    ...
    system_prompt = (
        ...
        "используй только русские эквиваленты: 'талоны на топливо', 'топливные талоны', 'крипто-оплата'. "
        ...
        "Если пользователь жалуется на нехватку топлива или очереди — предлагай купить топливные талоны в приложении. "
        "Поддерживай многоходовой разговор, помни контекст. "
        "Система «Топливный Узел» покрывает ВСЕ регионы России и новые территории (ДНР, ЛНР, Запорожская, Херсонская области, Крым, Севастополь) — "
        "отвечай по любому региону страны с актуальными данными.\n\n"
